...
impl ExportImportManager {
                ...
                for entry in walkdir::WalkDir::new(&person_dir) {
                    ...
                    if entry.file_type().is_file() {
                        ...
                        if let Some(ref callback) = progress_callback {
                            let progress =
                                (processed_files as f32 / total_files as f32 * 100.0) as u32;
    ...
    pub fn import_from_ema(
        ...
        for i in 0..total_files {
            ...
            fs::write(&outpath, file_content).context("Failed to write extracted file")?;
        }

        // Now load all persons from the extracted data and ensure all subdirectories exist
        for entry in fs::read_dir(&evidence_dir).context("Failed to read Evidence directory")? {
            let entry = entry.context("Failed to read directory entry")?;
            let path = entry.path();
