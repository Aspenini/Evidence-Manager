...
impl FileManager {
    ...
    pub fn create_person_folder(&self, person: &Person) -> Result<PathBuf> {
        ...
        if !person_folder.exists() {
            fs::create_dir_all(&person_folder).context("Failed to create person folder")?;
            ...
            for evidence_type in [
                ...
                fs::create_dir_all(&subfolder).context("Failed to create evidence subfolder")?;
            }
        }
    ...
    pub fn save_person_data(&self, person: &Person) -> Result<()> {
        ...
        let json =
            serde_json::to_string_pretty(person).context("Failed to serialize person data")?;

        fs::write(&person_data_file, json).context("Failed to write person data file")?;
    ...
    pub fn load_person_data(&self, person_folder: &Path) -> Result<Person> {
        ...
        let json =
            fs::read_to_string(&person_data_file).context("Failed to read person data file")?;

        let person: Person = serde_json::from_str(&json).context("Failed to parse person data")?;
    ...
    pub fn load_all_persons(&self) -> Result<Vec<Person>> {
        ...
        for entry in
            fs::read_dir(&self.evidence_dir).context("Failed to read Evidence directory")?
        {
    ...
    pub fn delete_person(&self, person: &Person) -> Result<()> {
        ...
        if person_folder.exists() {
            fs::remove_dir_all(&person_folder).context("Failed to delete person folder")?;
        }

        Ok(())
    }

    pub fn copy_file_to_evidence(
        &self,
        person: &Person,
        source_path: &Path,
        ...
        fs::copy(source_path, &final_path).context("Failed to copy file to evidence folder")?;

        let metadata = fs::metadata(&final_path).context("Failed to get file metadata")?;
    ...
    pub fn scan_person_evidence(&self, person: &Person) -> Result<Vec<EvidenceFile>> {
            ...
            .filter(|e| e.file_type().is_file())
        {
            ...
            if let Some(extension) = path.extension() {
                if let Some(evidence_type) =
                    EvidenceType::from_extension(extension.to_string_lossy().as_ref())
                {
